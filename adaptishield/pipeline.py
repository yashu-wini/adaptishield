...
class AdaptiShieldPipeline:
    ...
    def process_bytes(self, content: bytes, filename: str = "upload.pdf") -> dict:
        ...
        return self._run_pipeline(text, document_name=filename, document_type=suffix.lstrip("."))
    ...
    def _run_pipeline(
        ...
        validated = [self.context_validator.validate(d, text) for d in fused]
        validated = [d for d in validated if d.get("context_valid", True)]
        recalibrated = self.confidence_engine.recalibrate(validated)
        logger.info(f"  ✅ Stage 3: Context — {len(recalibrated)} entities after validation")
